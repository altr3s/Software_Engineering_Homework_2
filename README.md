...
- Тестирование функций `getMin()`, `getMax()`, `getSum()`, `getMult()`.
...
- `testMin`: проверка функции `getMin()`.
- `testMax`: проверка функции `getMax()`.
- `testSum`: проверка функции `getSum()`.
- `testMult`: проверка функции `getMult()`.
...
- `runTest`: выполняет тесты для функций `getMin()`, `getMax()`, `getSum()`, `getMult` и записывает время
...
##### Ниже вы можете найти график зависимости время выполнения функции (в мс) "getSum()" от количества чисел в файле и сводную таблицу.
##### Временная сложность алгоритма (функции "getSum()"): `O(n)`
